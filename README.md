...
- **`loadtest/`**: Нагрузочные тесты на базе k6.
  - [Подробнее о нагрузочном тестировании](./loadtest/README.md)
...
Замените `registry-name` на имя вашего репозитория в Artifact Registry во всех Dockerfile и скриптах деплоя.
...
Эта команда соберет и отправит базовые образы (`base-build` и `base-run`) в реестр `registry-name.cr.cloud.ru`.
...
Для проверки производительности системы под нагрузкой используйте k6:
...
Подробные инструкции по установке k6 и запуску различных сценариев доступны в [директории loadtest](./loadtest/README.md).
