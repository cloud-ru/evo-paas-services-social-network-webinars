...
Приложение использует **многоэтапную сборку Docker** с пользовательскими базовыми образами, хранящимися в частном реестре (`registry-name.cr.cloud.ru`). Эта архитектура оптимизирует время сборки и уменьшает размер образа.

### Базовые образы

Процесс сборки зависит от двух базовых образов:

1. **base-build** (`registry-name.cr.cloud.ru/base/frontend-build:latest`)
...
2. **base-run** (`registry-name.cr.cloud.ru/base/frontend-run:latest`)
   - Основан на `node:22-alpine`
...
yarn docker:tag     # Тегирование как registry-name.cr.cloud.ru/social-network-frontend:latest
...
```bash
docker run -p 3000:3000 registry-name.cr.cloud.ru/social-network-frontend:latest
```
